...
If you need a custom feature set or tag name, use `createPlayer()` to get `ProviderMixin` and define your own state boundary. Extend `ContainerElement` separately when you need custom container behavior:
...
If you omit `<video-skin>` and import only the player and media elements, those skin styles are not loaded. The provider still owns state, not layout, so add and style a `<media-container>` instead of relying on the provider's default display.
</FrameworkCase>
